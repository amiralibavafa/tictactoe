...
class MCTS: #Monte Carlo Tree Search implementation
    ...
    def monteCarloPlayer(self, timelimit = 4):
        ...
        while time.perf_counter() < end:
            selectedNode = self.selectNode(self.root) # selecting the best leafnode
            ...
            if not self.isTerminalState(selectedNode.state.utility, selectedNode.state.moves):
                ...
                if selectedNode.visitCount > 0:
                    ...
                    selectedNode = self.selectNode(selectedNode)

            winner = self.simulateRandomPlay(selectedNode) #simulating with random plays til we reach terminal state and get the winner
            #print("Winner in this simulation is : ", winner)
            self.backPropagation(selectedNode, winner) #backproping and updating the parents score and visited number accordingly
    ...
    def selectNode(self, nd):
        ...
        while len(node.children) != 0 : # Continue as long as the node has children
            node = self.findBestNodeWithUCT(node)

        #print("leafNode selection")

        # Code no2 forselecting node
        """node = self.findBestNodeWithUCT(node)
    ...
    def findBestNodeWithUCT(self, nd):
        ...
        #print("usingUCT")
        bestChild = None;
        bestChildUCT = float('-inf')

        for child in nd.children:
            currentUCT = self.uctValue(nd.visitCount, child.winScore, child.visitCount)

            if currentUCT > bestChildUCT :
                bestChild = child
                bestChildUCT = currentUCT
        return bestChild
    ...
    def uctValue(self, parentVisit, nodeScore, nodeVisit):
        ...
        # UCT formula
        exploitation = nodeScore / nodeVisit  # Average score
        exploration = math.sqrt(math.log(parentVisit) / nodeVisit)  # Exploration term
        C = self.exploreFactor  # Exploration constant
    ...
    def simulateRandomPlay(self, nd):
        ...
        while len(tempState.moves) >0:
            random_move=random.choice(tempState.moves)
            tempState = self.game.result(tempState, random_move)
            if self.game.terminal_test(tempState):
                break
        return 'X'if tempState.utility>0 else 'O' if tempState.utility<0 else 'N' #N signifies a tie
